...
st.markdown("Verifique se as referências citadas no seu texto existem em fontes confiáveis.")
...
if texto:
    ...
    refs = extrair_titulos_autores_simples(texto)

    st.markdown("### 🔍 Resultados da Verificação")

    for doi in dois:
        r = buscar_por_doi(doi)
        st.markdown(f"**🔗 DOI `{doi}` → {r['status'].upper()}** ({r['fonte']})")

    for isbn in isbns:
        r = buscar_por_isbn(isbn)
        st.markdown(f"**📘 ISBN `{isbn}` → {r['status'].upper()}** ({r['fonte']})")

    for ref in refs:
        entrada = f"{ref['autor']}: {ref['titulo']}"
        r = buscar_por_scielo(ref['titulo'], ref['autor'])
        st.markdown(f"**📝 {entrada} → {r['status'].upper()}** ({r['fonte']})")
